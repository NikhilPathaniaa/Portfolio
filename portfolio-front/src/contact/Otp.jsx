...
const onChange =(e) => 
  ...
  const handleSubmit = async (e) => {
    ...
    if (!response.ok) {
      ...
      setSubmitStatus("success"); // Set submit status to true when OTP verification is successful
      console.log(submitStatus);// Print submit status in console
    ...
    <div style={overlayStyles} ref={modelRef} onClick={onChange} >
      <div   id="portfiloOne" className=" modal-container popup" >
         <div className="pr-3 pb-2 ">
              ...
              <div className="mb-4">
                <span className="dark:text-white text-[25px] font-medium">Enter Your OTP:</span>
                  <input autoFocus value={otp}  onChange={(e) => setOtp(e.target.value)}
                    ...
                    style={{ border: '2px solid black' }} id="otp" type="number" placeholder="Enter OTP" required/>
              </div>
          ...
          <div className='text-center'>If you want to cancel the process <br/> <span className='text-[#FA5252]'>click outside the box 📤</span></div>
        </div>
